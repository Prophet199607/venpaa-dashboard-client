...
export default function FeaturedAuthorsPage() {
    ...
    try {
      ...
      const details = await Promise.all(
        rawItems.map(async ({ id, code }) => {
          let name = code;
          try {
            const { data: detailData } = await api.get(`/authors/${code}`);
            if (detailData.success && detailData.data) {
              name = detailData.data.auth_name || code;
            }
          } catch {
      ...
      toast({
        title: "Error",
        description: "Failed to load featured authors.",
        type: "error",
      });
    } finally {
      setLoading(false);
  ...
  const handleAddAuthor = useCallback(
    async (authCode: string) => {
      ...
      if (items.some((item) => item.code === authCode)) {
        toast({
          title: "Already added",
          description: "This author is already featured.",
          type: "warning",
        });
        return;
      }
      ...
      let name = authCode;
      try {
        const { data } = await api.get(`/authors/${authCode}`);
        if (data.success && data.data) {
          name = data.data.auth_name || authCode;
        }
      } catch {
        // use code as name
      }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({
        title: "Success",
        description: "Featured authors updated successfully.",
        type: "success",
      });
      setHasChanges(false);
        ...
        <div className="flex items-center gap-4">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">
              Manage featured authors displayed on the website.
            </p>
          </div>
        </div>
        ...
        <div className="w-full">
          <Card className="border-none shadow-md bg-gradient-to-br from-white to-neutral-50/50 dark:from-neutral-900 dark:to-neutral-950">
            <CardHeader className="pb-3">
              ...
              <CardDescription>
                Search for authors to feature on the website.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
                <div className="md:col-span-2">
                  <AuthorSearch
                    onValueChange={(value) => handleAddAuthor(value)}
                  />
                </div>
        ...
        <div className="w-full">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-8 gap-2 text-muted-foreground">
              ...
              <p>Loading featured authors...</p>
            </div>
          ) : items.length === 0 ? (
            <Card className="border-dashed h-[400px] flex flex-col items-center justify-center text-center p-4">
              ...
              <h3 className="text-base font-semibold">
                No authors featured yet
              </h3>
              <p className="text-muted-foreground text-xs max-w-sm mt-2">
                Use the search box above to find and add authors to the featured
                section.
              </p>
            </Card>
          ) : (
