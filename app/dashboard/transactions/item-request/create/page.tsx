...
type FormData = z.infer<typeof itemRequestSchema>;
...
function ItemRequestFormContent() {
  ...
  const generateIrNumber = async (
    ...
    try {
      ...
      console.error("Failed to generate IR number:", error);
    } finally {
  ...
  useEffect(() => {
    ...
    const checkUnsavedSessions = async () => {
      try {
        const { data: res } = await api.get("/item-requests/unsaved-sessions");
  ...
  useEffect(() => {
    ...
    const loadItemRequest = async () => {
      ...
      try {
        ...
        console.error("Failed to load item request:", error);
      } finally {
        setFetching(false);
      }
    };
  ...
  const onSubmit = (values: FormData) => {
    if (isEditMode) {
      handleUpdateDraftItemRequest(values);
    } else {
      handleCreateDraftItemRequest(values);
    }
  };
  ...
  const handleCreateDraftItemRequest = async (values: FormData) => {
    ...
    try {
      ...
      if (response.data.success) {
        toast({
          title: "Success",
          description: "Item request has been drafted successfully.",
          type: "success",
        });
        router.push("/dashboard/transactions/item-request");
      }
    } catch (error: any) {
      console.error("Failed to draft IR:", error);
      toast({
        title: "Operation Failed",
        description: error.response?.data?.message || "Could not draft the IR.",
        type: "error",
      });
    } finally {
      setLoading(false);
  ...
  const handleUpdateDraftItemRequest = async (values: FormData) => {
    ...
    try {
      ...
      if (response.data.success) {
        toast({
          title: "Success",
          description: "Item Request has been updated successfully.",
          type: "success",
        });
        router.push("/dashboard/transactions/item-request");
      }
      ...
      toast({
        title: "Operation Failed",
        description:
          error.response?.data?.message || "Could not update the IR.",
        type: "error",
      });
    } finally {
      setLoading(false);
