...
function PaymentVoucherListingContent() {
  ...
  const fetchVouchers = useCallback(async () => {
    try {
      setFetching(true);
      const { data: res } = await api.get("/payment-vouchers/load-all-pmt", {
        params: {
      ...
      const formattedData: PaymentVoucher[] = res.data.map((item: any) => ({
...
export default function PaymentVoucherListingPage() {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <PaymentVoucherListingContent />
    </Suspense>
  );
}
