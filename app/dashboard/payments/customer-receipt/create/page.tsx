...
import { ReceiptSetOffModal } from "@/components/model/payments/receipt-set-off-modal";
...
export default function CustomerReceiptPage() {
  ...
  const generateRecNumber = useCallback(
    async (locaCode: string, setFetchingState = true): Promise<string> => {
      try {
        ...
        const { data: res } = await api.post(
          `/customer-receipts/generate-code`,
          { loca: locaCode },
        );
        ...
        console.error("Failed to generate REC number:", error);
        toast({
          title: "Error",
  ...
  useEffect(() => {
    const fetchPendingReceipts = async () => {
      if (!customer || !selectedLocation) return;
      try {
        const { data: res } = await api.get(
          `/customer-receipts/pending-receipts/${customer}/${selectedLocation}/INV`,
        );
        if (res.success) {
  ...
  useEffect(() => {
    const fetchAvailableSetOffsRec = async () => {
      if (!customer || !selectedLocation) {
        setSetOffBalance("");
        return;
      }
      try {
        const { data: res } = await api.get(
          `/customer-receipts/available-set-offs/${customer}/${selectedLocation}`,
        );
  ...
  const onSubmit = async (data: CustomerReceiptFormValues) => {
    if (!customer || !selectedLocation) {
      toast({
        title: "Validation Error",
        description: "Please select customer and location.",
        type: "error",
      });
      return;
    }
    ...
    try {
      ...
      if (res.success) {
        toast({
          title: "Success",
          description: res.message || "Customer receipt created successfully.",
          type: "success",
        });

        handleClear();
      } else {
        throw new Error(res.message || "Failed to create customer receipt.");
      }
    } catch (error: any) {
      ...
      toast({
        ...
        description:
          ...
          error.message ||
          "Failed to create customer receipt. Please try again.",
        type: "error",
      });
    }
  };
  ...
  useEffect(() => {
    const fetchCustomerName = async () => {
      ...
      try {
        const { data: res } = await api.get(
          `/customers/search?query=${encodeURIComponent(customer)}`,
        );
        if (res.success && res.data.length > 0) {
          const foundCustomer = res.data.find(
            (c: any) => c.customer_code === customer,
          );
          if (foundCustomer) {
