...
interface PrintSupplierReturnNoteContentProps {
  docNo: string;
  status: string;
  iid?: string;
  initialData?: any;
...
export default function PrintSupplierReturnNoteContent({
  docNo,
  status,
  iid,
  initialData,
  onLoad,
}: PrintSupplierReturnNoteContentProps) {
  ...
  if (!data) {
    return (
      <div className="p-8">
        <p>Supplier return note transaction not found.</p>
      </div>
    );
  }
    ...
    <div className="p-4">
      <div className="mb-5">
        <div className="flex justify-between items-start">
          <h2 className="text-xl font-bold mb-5">SUPPLIER RETURN NOTE</h2>
      ...
      <div>
        <p className="mt-4">
          <strong>SRN Remarks:</strong> {data.srn_remarks || "No remarks"}
        </p>
      </div>
    </div>
  );
