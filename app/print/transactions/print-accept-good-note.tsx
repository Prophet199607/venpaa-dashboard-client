...
interface PrintAcceptGoodNoteContentProps {
  docNo: string;
  status: string;
  iid?: string;
  initialData?: any;
...
export default function PrintAcceptGoodNoteContent({
  docNo,
  status,
  iid,
  initialData,
  onLoad,
}: PrintAcceptGoodNoteContentProps) {
    ...
    const fetchAcceptGoodNote = async () => {
      try {
        ...
        if (res.success) {
          ...
          setError("Failed to load accept good note transactioin data");
        }
      } catch (error) {
        console.error(
          "Failed to fetch accept good note transactioin for printing:",
          error
        );
        setError("Failed to load accept good note transactioin data");
      } finally {
  ...
  if (!data) {
    return (
      <div className="p-8">
        <p>Accept good note transaction not found.</p>
      </div>
    );
  }
    ...
    <div className="p-4">
      <div className="mb-5">
        <div className="flex justify-between items-start">
          <h2 className="text-xl font-bold mb-5">ACCEPT GOOD NOTE</h2>
