...
interface ReceiptSetOffModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (data: {
...
export function ReceiptSetOffModal({
  isOpen,
  onClose,
  onConfirm,
  documents,
  ...
  useEffect(() => {
    if (isOpen) {
        ...
        const fetchCredits = async () => {
          try {
            const { data: res } = await api.get(
              `/customer-receipts/available-set-offs/${customerCode}/${locationCode}`
            );
            if (res.success) {
      ...
      setDescription(
        customerName ? `Receipt set off for ${customerName}` : "Receipt set off"
      );
    }
  }, [isOpen, documents, customerName, customerCode, locationCode]);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Receipt Set Off</DialogTitle>
        </DialogHeader>
