...
import PrintGoodReceiveNoteContent from "@/app/print/transactions/print-good-receive-note";
...
interface ViewGoodReceiveNoteProps {
  isOpen: boolean;
  onClose: () => void;
  docNo: string;
  status: string;
...
export default function ViewGoodReceiveNote({
  isOpen,
  onClose,
  docNo,
  status,
  iid,
}: ViewGoodReceiveNoteProps) {
  ...
  useEffect(() => {
    const fetchGoodReceiveNote = async () => {
      try {
        ...
        console.error("Failed to fetch good receive note:", error);
        toast({
          title: "Error",
          description:
            error.response?.data?.message || "Failed to load good receive note",
          type: "error",
        });
      } finally {
        setLoading(false);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
        <DialogTitle hidden={isOpen}>Good Receive Note Details</DialogTitle>
        <DialogDescription hidden={isOpen}>
          Detailed view of a good receive note, including products and summary.
        </DialogDescription>
        <div className="absolute right-4 top-4 flex gap-2">
          {!permissionsLoading && hasPermission("print good-receive-note") && (
            <button
              onClick={handlePrint}
        ...
        <div className="space-y-6 py-4 mt-3">
          ...
          <div className="flex justify-between items-start">
            <div>
              <h2 className="text-lg font-semibold">GOOD RECEIVE NOTE</h2>
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="font-semibold">
              ...
              <p>
                Invoice Amount:{" "}
                <span className="font-normal"> {data.invoice_amount}</span>
              </p>
            </div>
          </div>
