...
interface ViewTransferGoodReturnProps {
  isOpen: boolean;
  onClose: () => void;
  docNo: string;
  status: string;
...
export default function TransferGoodReturn({
  isOpen,
  onClose,
  docNo,
  status,
  iid,
}: ViewTransferGoodReturnProps) {
  ...
  useEffect(() => {
    const fetchTransferGoodReturn = async () => {
      try {
        setLoading(true);
        ...
        console.error("Failed to fetch transfer good return:", error);
        toast({
          title: "Error",
          description:
            error.response?.data?.message ||
            "Failed to load transfer good return",
          type: "error",
        });
      } finally {
        setLoading(false);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
        <DialogTitle hidden={isOpen}>Transfer Good Return Details</DialogTitle>
        <DialogDescription hidden={isOpen}>
          Detailed view of a transfer good return, including products and
          summary.
        </DialogDescription>
        <div className="absolute right-4 top-4 flex gap-2">
          {!permissionsLoading && hasPermission("print transfer-good-return") && (
            <button
              onClick={handlePrint}
        ...
        <div className="space-y-6 py-4 mt-3">
          ...
          <div className="flex justify-between items-start">
            <div>
              <h2 className="text-lg font-semibold">TRANSFER GOOD RETURN</h2>
